...
- Confirm waitlist/contact copy says beta watch requests are available, while fully automated alert sending comes later.
- Confirm My Focus can submit a beta watch setup by email or text, shows the watch preview, and clearly says candidates are reviewed before sending.
...
10. If reviewed email delivery is configured, dry-run one candidate send before inviting students.
11. Confirm test entries can be identified or removed from the capture destination before inviting students.
...
- The tester knows beta watch requests are reviewed before any alert is sent, unless automatic sending is deliberately enabled later.
