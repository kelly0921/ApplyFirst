...
- Review `docs/VERIFIED_SEED_SCHEDULE_AUDIT.md` if opportunity URLs, statuses, or expected opening months changed.
...
18. Open the unsubscribe link from a test email and confirm the watch request status changes to `unsubscribed` before testing with another student.
19. Confirm test entries can be identified or removed from the capture destination before inviting students.
